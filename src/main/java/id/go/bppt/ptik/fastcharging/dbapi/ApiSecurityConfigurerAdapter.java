...
import id.go.bppt.ptik.fastcharging.dbapi.config.APIKeyAuthFilter;

@EnableWebSecurity
@Configuration
public class ApiSecurityConfigurerAdapter extends WebSecurityConfigurerAdapter {
	@Value("${sonikapp.http.auth-token-header-name}")
	private String principalRequestHeader;

	@Value("${sonikapp.http.auth-token-value}")
	...
	protected void configure(HttpSecurity httpSecurity) throws Exception {
		...
		httpSecurity.antMatcher("/api/backend/**").csrf().disable().sessionManagement()
